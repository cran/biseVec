...
gauss <-
function(ndvi, globalthreshold=0, asym=FALSE, resvec=c(0.55,0.6,0.65,0.65,0.7)){
	days <- length(ndvi)
	...
	if (asym){
		...
		model.nls <- try(nls(res ~ G(mu=mu, sig=sig, scal=scal, base=base, time=time, days=days), 
		...
		if (inherits(model.nls, "try-error")==FALSE){
			...
			ndvi <- ifelse(is.na(ndvi), -1, ndvi)
			model <- .C("gauss", rdays=as.integer(days), ndvi=as.numeric(ndvi),
				mustart=as.integer(mu), sigstart=as.numeric(sig), 
				rbase=as.numeric(base), model=as.numeric(model), 
				PACKAGE="biseVec")$model
		}
